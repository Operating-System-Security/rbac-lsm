...
数据库中存储若干这样的五元组，当 $U_i$ 通过 $Op_j$ 访问 $O_k$ 时，检查数据库中是否存在 $(U_i, R_l, A_m, Op_j, O_k)$，若存在且 $A_m$ 为允许，**或四元组不存在**，则允许访问；若存在且 $A_m$ 为拒绝，则拒绝访问。
...
本实验基于 Linux 5.14.0-rc2 实现，使用 BusyBox 制作了根文件系统，并且在 LoongArch 32 Reduced 架构的 QEMU 平台上进行了验证。
